from const import coeffs_count, art_vis_coeffs_count
from countData import calculate_p, runge_error
from graphics import graphics13, graphics_relative_errors_and_local_orders, graphics
...
def task13():
    ...
    # for T in times:
    a_coeffs = coeffs_count(6)
    art_vis_coeffs = art_vis_coeffs_count(6)
    x, h, q = start_NFD(x_start, x_end, X, a_coeffs, art_vis_coeffs, N, 0.5, a, b, 6)
    graphics(x, h)
...
def task_relative_errors_and_local_orders_1():
    X = 10
    a = 2
    b = 10
    x_start = 0
    ...
    for T in times:
        ...
        print(f"T = {T}")
        for order in orders:
            a_coeffs = coeffs_count(order)
            art_vis_coeffs = art_vis_coeffs_count(order)
            t1 = datetime.now()
            print(f"start N = {N[0]}")
            x1, h1, q1 = start_NFD(x_start, x_end, X, a_coeffs, art_vis_coeffs, N[0], T, a, b, order)
            t2 = datetime.now()
            print(f"Count NFD on {N[0]}-dot grid is {t2 - t1}, order is {order}")

            print(f"start N = {N[1]}")
            t1 = datetime.now()
            x2, h2, q2 = start_NFD(x_start, x_end, X, a_coeffs, art_vis_coeffs, N[1], T, a, b, order)
            t2 = datetime.now()
            print(f"Count NFD on {N[1]}-dot grid is {t2 - t1}, order is {order}")

            print(f"start N = {N[2]}")
            t1 = datetime.now()
            x3, h3, q3 = start_NFD(x_start, x_end, X, a_coeffs, art_vis_coeffs, N[2], T, a, b, order)
            t2 = datetime.now()
